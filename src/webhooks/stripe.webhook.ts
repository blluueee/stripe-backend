  ...
  try {
    ...
    case "checkout.session.completed":
      ...
      if(session.mode === "subscription"){
        ...
        const savedSub = await Subscription.findOneAndUpdate({subscriptionId : subscription.id},
          { userId,
            ...
            isTrial: subscription.status === "trialing", isTrialUsed: true
           }, {upsert: true, new: true}
        )
         await User.findByIdAndUpdate(userId, { subscriptionId: savedSub._id})
      }
    ...
    case "payment_intent.succeeded":{
      ...
      await generateInvoice( paymentIntent.customer as string, paymentIntent.amount, "Product purchase")
    }
      break;
    ...
    case "invoice.payment_succeeded": {
      ...
      await Subscription.findOneAndUpdate({ subscriptionId: stripeSubscriptionId}, 
        {status: "active"}
      )}
      break;
      ...
      await Subscription.findOneAndUpdate({ subscriptionId: invoice.subscription as string},
      { status: "canceled" })}
      break;
    ...
    case "customer.subscription.deleted": {
      ...
      await Subscription.findOneAndUpdate({ subscriptionId: subscription.id},
